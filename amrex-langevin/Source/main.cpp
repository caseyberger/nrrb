...
=======
        ...
        // If using HDF5, should we append output into the same plotfiles grouped by step?
        pp.query("append_hdf5", nrrb_parm.append_hdf5);

>>>>>>> add an option (on by default with HDF5) to append HDF5 output into the same file, grouped by step number
